export const TERRITORIOS = Object.freeze({
  "Bohemia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Hungría": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Croacia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Polonia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Castilla": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "pertenencias": [
      {
        "territorio": "Corona de Castilla",
        "desde": 1230
      }
    ]
  },
  "León": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "pertenencias": [
      {
        "territorio": "Corona de Castilla",
        "desde": 1230
      }
    ]
  },
  "Aragón": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "pertenencias": [
      {
        "territorio": "Corona de Aragón",
        "desde": 1137
      }
    ]
  },
  "Navarra": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Portugal": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Francia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Inglaterra": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Escocia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Irlanda": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "La ficha distingue la supremacía del alto rey medieval del reino establecido en 1542. Ninguno de esos títulos implica por sí solo control efectivo de toda la isla. Los reinos y señoríos gaélicos tienen fichas y filtros propios.",
    "etapas": [
      {
        "desde": null,
        "clase": "reino"
      },
      {
        "desde": 1177,
        "clase": "señorío"
      },
      {
        "desde": 1542,
        "clase": "reino"
      }
    ]
  },
  "Dinamarca": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Noruega": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Suecia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Sicilia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Hasta 1282, reino que comprendía la isla y el sur continental. Después de las Vísperas, consultar Nápoles para el continente y Trinacria para la corona insular."
  },
  "Trinacria": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Denominación usada aquí para seguir la corona insular desde 1282. El título de Trinacria se formalizó con Caltabellotta y la ratificación papal de 1303; no fue el nombre único de la isla desde el comienzo."
  },
  "Nápoles": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Nombre historiográfico para el reino continental separado de la isla en 1282. Sus monarcas siguieron utilizando el título de reyes de Sicilia."
  },
  "Mallorca": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Valencia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "pertenencias": [
      {
        "territorio": "Corona de Aragón",
        "desde": 1238
      }
    ]
  },
  "Cerdeña": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Jerusalén": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Chipre": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": 1192,
        "clase": "señorío"
      },
      {
        "desde": 1197,
        "clase": "reino"
      }
    ]
  },
  "Armenia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Sirmia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Anjou": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "condado"
      },
      {
        "desde": 1360,
        "clase": "ducado"
      }
    ]
  },
  "Alençon": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Aquitania": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Baviera": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Berg": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Berry": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Borgoña": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Brabante": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Bretaña": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Brunswick": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Carintia": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Cléveris": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Curlandia": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Gandía": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": [],
    "pertenencias": [
      {
        "territorio": "Corona de Aragón",
        "desde": 1399
      }
    ]
  },
  "Guisa": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Güeldres": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Holstein": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Jülich": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Limburgo": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Lorena": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Mecklemburgo": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Milán": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "señorío"
      },
      {
        "desde": 1395,
        "clase": "ducado"
      }
    ]
  },
  "Módena": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Orleans": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Parma": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Pomerania": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Romaña": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Saboya": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "condado"
      },
      {
        "desde": 1416,
        "clase": "ducado"
      }
    ]
  },
  "Sajonia-Lauenburgo": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Schleswig": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Suabia": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Suffolk": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Urbino": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "York": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Bisceglie": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Gravina": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Durazzo": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Angulema": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Armagnac": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Artois": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Auvernia": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Boulogne": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Carrick": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Champaña": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Clermont": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Flandes": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Foix": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Ginebra": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Habsburgo": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Henao": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Hohenberg": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Holanda": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Huntingdon": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Kyburg": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Mark": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Montbéliard": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Namur": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Nassau": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Oldemburgo": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Pfirt": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Ponthieu": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Provenza": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Richmond": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Saint-Pol": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Tirol": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Trípoli": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Urgel": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Valois": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Vendôme": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "condado"
      },
      {
        "desde": 1515,
        "clase": "ducado"
      }
    ]
  },
  "Évreux": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Ansbach": {
    "clase": "margraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Baden": {
    "clase": "margraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Kulmbach": {
    "clase": "margraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Meißen": {
    "clase": "margraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Moravia": {
    "clase": "margraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Hesse": {
    "clase": "landgraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Hesse-Kassel": {
    "clase": "landgraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Hesse-Darmstadt": {
    "clase": "landgraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Turingia": {
    "clase": "landgraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Anhalt": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Antioquía": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Beloózero": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Gales": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Hannover": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "principado"
      },
      {
        "desde": 1692,
        "clase": "electorado"
      }
    ]
  },
  "Mazovia": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Moscú": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Piamonte": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Pólotsk": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Silesia": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Squillace": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Tarento": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Transilvania": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "voivodato"
      },
      {
        "desde": 1570,
        "clase": "principado"
      }
    ],
    "nota": "El voivodato dependía de la monarquía húngara. El principado se desarrolló bajo soberanía otomana y después habsbúrgica; en la etapa Báthory coexistieron príncipe y voivoda delegado."
  },
  "Vladímir": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Annandale": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Borbón": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "señorío"
      },
      {
        "desde": 1327,
        "clase": "ducado"
      }
    ]
  },
  "Forlì": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Galloway": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Ibelin": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Imola": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Ischia": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Montpellier": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Nablus": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Pesaro": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Torón": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Transjordania": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Florencia": {
    "clase": "republica",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Génova": {
    "clase": "republica",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Siena": {
    "clase": "republica",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Venecia": {
    "clase": "republica",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Anagni": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Brescia": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Camerino": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Cervia": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Cesena": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Como": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Edesa": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Epiro": {
    "clase": "despotado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Fano": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Lacio": {
    "clase": "grupo_geografico",
    "naturaleza": "agrupacion",
    "componentes": []
  },
  "Liguria": {
    "clase": "grupo_geografico",
    "naturaleza": "agrupacion",
    "componentes": []
  },
  "Northumbria": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Núremberg": {
    "clase": "burgraviato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Perugia": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Roma": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Rímini": {
    "clase": "ciudad",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Austria": {
    "clase": "archiducado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "ducado"
      },
      {
        "desde": 1453,
        "clase": "archiducado"
      }
    ]
  },
  "Austria Interior": {
    "clase": "territorio_compuesto",
    "naturaleza": "compuesta",
    "componentes": [
      "Carintia"
    ],
    "nota": "Conjunto dinástico de Estiria, Carintia y Carniola; solo se enlazan aquí los componentes presentes en la base."
  },
  "Brasil": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "reino"
      },
      {
        "desde": 1822,
        "clase": "imperio"
      }
    ]
  },
  "Bulgaria": {
    "clase": "imperio",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Rusia": {
    "clase": "imperio",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "gran_principado"
      },
      {
        "desde": 1547,
        "clase": "zarato"
      },
      {
        "desde": 1721,
        "clase": "imperio"
      }
    ]
  },
  "Imperio otomano": {
    "clase": "sultanato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Egipto": {
    "clase": "sultanato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Siria": {
    "clase": "sultanato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Sacro Imperio": {
    "clase": "imperio",
    "naturaleza": "compuesta",
    "componentes": [
      "Alemania",
      "Austria",
      "Bohemia",
      "Baviera",
      "Sajonia",
      "Brandeburgo",
      "Palatinado",
      "Lorena",
      "Saboya",
      "Milán"
    ],
    "nota": "Componentes destacados a lo largo de su historia; su pertenencia y autonomía variaron. No implica dominio hereditario del emperador."
  },
  "Alemania": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Imperio Latino": {
    "clase": "imperio",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Bizancio": {
    "clase": "imperio",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Bosnia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "banato"
      },
      {
        "desde": 1377,
        "clase": "reino"
      }
    ]
  },
  "Serbia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "principado"
      },
      {
        "desde": 1217,
        "clase": "reino"
      },
      {
        "desde": 1346,
        "clase": "imperio"
      },
      {
        "desde": 1371,
        "clase": "principado"
      }
    ]
  },
  "Valaquia": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Moldavia": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Granada": {
    "clase": "emirato",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Lituania": {
    "clase": "gran_ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Toscana": {
    "clase": "gran_ducado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Prusia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "ducado"
      },
      {
        "desde": 1701,
        "clase": "reino"
      }
    ]
  },
  "Brandeburgo": {
    "clase": "electorado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Palatinado": {
    "clase": "electorado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Sajonia": {
    "clase": "electorado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "ducado"
      },
      {
        "desde": 1356,
        "clase": "electorado"
      }
    ]
  },
  "Luxemburgo": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "condado"
      },
      {
        "desde": 1354,
        "clase": "ducado"
      }
    ]
  },
  "Württemberg": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "condado"
      },
      {
        "desde": 1495,
        "clase": "ducado"
      }
    ]
  },
  "Ferrara": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "señorío"
      },
      {
        "desde": 1471,
        "clase": "ducado"
      }
    ]
  },
  "Mantua": {
    "clase": "ducado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "señorío"
      },
      {
        "desde": 1433,
        "clase": "marquesado"
      },
      {
        "desde": 1530,
        "clase": "ducado"
      }
    ]
  },
  "Monferrato": {
    "clase": "marquesado",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "marquesado"
      },
      {
        "desde": 1574,
        "clase": "ducado"
      }
    ]
  },
  "Estados Pontificios": {
    "clase": "estado_pontificio",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Países Bajos": {
    "clase": "territorio_compuesto",
    "naturaleza": "compuesta",
    "componentes": [],
    "nota": "Ámbito histórico de instituciones cambiantes. Los estatuderatos son provinciales; la soberanía habsbúrgica y la República no son gobiernos equivalentes."
  },
  "Rus de Kiev": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "La Marche": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Bearne": {
    "clase": "vizcondado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Morea": {
    "clase": "despotado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Condado de Borgoña": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": []
  },
  "Corona de Castilla": {
    "clase": "corona_compuesta",
    "naturaleza": "compuesta",
    "componentes": [
      "Castilla",
      "León"
    ],
    "desde": 1230
  },
  "Corona de Aragón": {
    "clase": "corona_compuesta",
    "naturaleza": "compuesta",
    "componentes": [
      "Aragón",
      "Valencia",
      "Mallorca"
    ],
    "desde": 1137
  },
  "Polonia-Lituania": {
    "clase": "union_compuesta",
    "naturaleza": "compuesta",
    "componentes": [
      "Polonia",
      "Lituania"
    ],
    "desde": 1569
  },
  "España": {
    "clase": "grupo_historico",
    "naturaleza": "agrupacion",
    "componentes": [
      "Corona de Castilla",
      "Corona de Aragón",
      "Navarra",
      "Granada"
    ]
  },
  "Balcanes": {
    "clase": "grupo_geografico",
    "naturaleza": "agrupacion",
    "componentes": [
      "Bulgaria",
      "Serbia",
      "Bosnia",
      "Valaquia",
      "Moldavia",
      "Croacia"
    ]
  },
  "Escandinavia": {
    "clase": "grupo_geografico",
    "naturaleza": "agrupacion",
    "componentes": [
      "Dinamarca",
      "Noruega",
      "Suecia"
    ]
  },
  "Estados Italianos": {
    "clase": "grupo_geografico",
    "naturaleza": "agrupacion",
    "componentes": [
      "Milán",
      "Venecia",
      "Génova",
      "Florencia",
      "Toscana",
      "Mantua",
      "Ferrara",
      "Módena",
      "Parma",
      "Urbino",
      "Monferrato",
      "Saboya",
      "Nápoles",
      "Sicilia",
      "Trinacria",
      "Estados Pontificios"
    ]
  },
  "Países Bajos y Flandes": {
    "clase": "grupo_geografico",
    "naturaleza": "agrupacion",
    "componentes": [
      "Países Bajos",
      "Flandes",
      "Brabante",
      "Henao",
      "Holanda"
    ]
  },
  "Bizancio y Oriente latino": {
    "clase": "grupo_geografico",
    "naturaleza": "agrupacion",
    "componentes": [
      "Bizancio",
      "Imperio Latino",
      "Jerusalén",
      "Chipre"
    ]
  },
  "Georgia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Reino medieval de los Bagrationi. Sus sucesores de la Edad Moderna tienen fichas propias; no se prolonga artificialmente una única sucesión sobre toda Georgia."
  },
  "Imericia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Reino occidental surgido de la separación de David VI en 1259; reunificado y vuelto a separar en etapas posteriores."
  },
  "Samtsje": {
    "clase": "principado",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Señorío de los Jakeli, conocido también como Samtskhe. El rango de príncipe aquí normaliza el señorío hereditario; no atribuye retroactivamente el título de atabeg a cada gobernante."
  },
  "Trebisonda": {
    "clase": "imperio",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Imperio de los Grandes Comnenos del mar Negro, distinto del Imperio bizantino de Constantinopla."
  },
  "Kartli": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Reino de Georgia oriental, con soberanía condicionada por Persia y por las intervenciones otomanas."
  },
  "Kajetia": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "También Kakheti. Reino oriental de los Bagrationi, unido a Kartli en 1762."
  },
  "Kartli-Kajetia": {
    "clase": "reino",
    "naturaleza": "compuesta",
    "componentes": [
      "Kartli",
      "Kajetia"
    ],
    "desde": 1762,
    "nota": "Unión de los dos reinos orientales desde 1762; no incluye Imericia ni equivale a toda Georgia."
  },
  "Georgia y Cáucaso": {
    "clase": "grupo_geografico",
    "naturaleza": "agrupacion",
    "componentes": [
      "Georgia",
      "Imericia",
      "Samtsje",
      "Kartli",
      "Kajetia",
      "Kartli-Kajetia"
    ],
    "nota": "Agrupación de navegación: los reinos conservan sus cronologías y no comparten una soberanía permanente."
  },
  "Connacht": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Reino gaélico del oeste de Irlanda. Su continuidad política no termina con la llegada anglonormanda de 1169."
  },
  "Leinster": {
    "clase": "reino",
    "naturaleza": "entidad",
    "componentes": [],
    "etapas": [
      {
        "desde": null,
        "clase": "reino"
      },
      {
        "desde": 1171,
        "clase": "señorío"
      }
    ],
    "nota": "La realeza gaélica y el señorío anglonormando representaron legitimidades distintas; la conquista no supone una transferencia automática del título de rey."
  },
  "Tír Eoghain": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Señorío gaélico de los O’Neill. El liderazgo del linaje y el condado de Tyrone creado por la Corona se registran por separado."
  },
  "Tír Chonaill": {
    "clase": "señorío",
    "naturaleza": "entidad",
    "componentes": [],
    "nota": "Señorío gaélico de los O’Donnell, conocido también como Tyrconnell. No es equivalente al condado actual de Donegal."
  },
  "Condado de Tyrone": {
    "clase": "condado",
    "naturaleza": "entidad",
    "componentes": [],
    "desde": 1542,
    "nota": "Título de la nobleza irlandesa creado por Enrique VIII. Se distingue del territorio gaélico de Tír Eoghain y de sus reglas sucesorias."
  }
});

export const TITULOS_POR_CLASE = {"reinado": ["Rey", "Rey de Romanos", "Alto rey", "Reina"], "imperio": ["Emperatriz", "Emperador"], "ducado": ["Duquesa", "Duque"], "archiducado": ["Archiduque", "Archiduquesa"], "condado": ["Conde", "Conde palatino", "Condesa"], "electorado": ["Elector"], "principado": ["Princesa", "Gran župan", "Príncipe"], "gran_ducado": ["Gran duque", "Gran duquesa"], "gran_principado": ["Gran príncipe"], "zarato": ["Zarina", "Zar"], "margraviato": ["Margrave"], "marquesado": ["Marqués"], "landgraviato": ["Landgrave"], "señorío": ["Señor", "Señora"], "regencia": ["Regente"], "estatuderato": ["Estatúder"], "emirato": ["Emir"], "sultanato": ["Sultán"], "despotado": ["Déspota"], "banato": ["Ban"], "voivodato": ["Voivoda"], "pontificado": ["Papa"], "gobierno": ["Gobernante", "Soberano", "Soberana", "Gobernador", "Gobernadora"], "vizcondado": ["Vizconde"]};
export const CONDICIONES = ["efectivo","jure uxoris","corregente","regencia","pretensión","titular","rival","disputado","rama"];
export function componentesDe(nombre, vistos = new Set()) {
 if(vistos.has(nombre)) return []; vistos.add(nombre);
 return [...new Set((TERRITORIOS[nombre]?.componentes || []).flatMap(n => [n,...componentesDe(n,vistos)]))];
}
export function gobiernoEfectivo(g) { return g.efectivo !== false && !["pretensión","titular","rival","disputado"].includes(g.condicion); }
