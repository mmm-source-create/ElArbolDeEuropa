...
export function TerritoryPage({slug}) {
 ...
 <div className="territory-actions"><a className="public-primary" href={atlas}>Abrir en el Atlas →</a><a className="public-secondary" href={`${atlas}&vista=sucesion`}>Explorar Sucesión →</a></div></header>
 {t.nota&&<p>{t.nota}</p>}
 ...
 <section><h2>Gobiernos registrados</h2><p>Incluye regencias, ramas, títulos nominales y disputas cuando constan. Los intervalos son anuales y no implican continuidad ni exhaustividad.</p>
 {!t.gobiernos.length?<p>No hay gobiernos cargados para esta entidad. Consulta las personas relacionadas.</p>:<div className="territory-table"><table><thead><tr><th>Persona</th><th>Territorio y título</th><th>Periodo</th><th>Condición</th></tr></thead><tbody>{t.gobiernos.map((g,i)=><tr key={i}><td><a href={`/es/persona/${g.persona.slug}`}>{g.persona.nombre}</a></td><td>{g.titulo} · {g.territorio}<small>{claseTexto(g.clase)}</small></td><td>{g.desde}–{g.hasta}</td><td>{g.condicion}{g.ambito&&` · ${g.ambito}`}{g.nota&&<small>{g.nota}</small>}</td></tr>)}</tbody></table></div>}</section>
